...
InputGSVAFile = FPKM_GSVA_Scored;Percentage = 0.25
...
## T Cell States
Scale = 2
pdf(file = "7A_TCGA Survival_TCellStates.pdf",width = 7.2*Scale,height = 1.3*Scale)
PlotSurvival(read.gmt("../../TCellSigs_NE.gmt")$genesets[c(1,5,9,7,11,3)],ClinicalInfo_Survival)
dev.off()


# Test = PlotSurvival(read.gmt("../../YellowFever_EffectorMemory_NE.gmt")$genesets[c(1,3)],ClinicalInfo_Survival,Vertical = TRUE)

## YF Microarray NE
pdf(file = "7DE_TCGA Survival_YFMicroarray NE.pdf",width = 1.7*Scale,height = 2.4*Scale)
PlotSurvival(read.gmt("../../YellowFever_EffectorMemory_NE.gmt")$genesets[c(1,3)],ClinicalInfo_Survival,Vertical = TRUE)
dev.off()

## Jansen Stem-like
pdf(file = "7DE_TCGA Survival_Jansen.pdf",width = 1.7*Scale,height = 2.4*Scale)
PlotSurvival(read.gmt("../../HumanTILsSigs.gmt")$genesets[c(2,1)],ClinicalInfo_Survival, Vertical = TRUE)
dev.off()

## YF RNASeq NE
pdf(file = "S7G_TCGA Survival_YFRNAseq NE.pdf",width = 1.7*Scale,height = 2.4*Scale)
