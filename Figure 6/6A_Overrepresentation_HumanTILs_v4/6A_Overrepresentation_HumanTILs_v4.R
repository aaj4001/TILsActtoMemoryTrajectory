...
PicnicPlot <- function(InputSigList,InputQueryList,FDRCutoff = 0.05,
  ...
  if(FlipVertical){
    PicPlot = PicPlot + coord_flip() + 
      ...
      scale_x_discrete(limits = rev(levels(InputResults$Test_Signatures)),position = "bottom")
    ...
    PicPlot = PicPlot +
      theme(axis.text.x = element_text(angle = 90,vjust =0.5,hjust = 0),axis.title = element_blank()) +
      scale_y_discrete(limits = rev(levels(InputResults$TCellState_Sigs)),position = "left") +
...
## 5A Jansen Scoring to T Cell state signatures
Human_TILs_Sigs = msigdb::read.gmt("../../HumanTILsSigs.gmt")$genesets
JansenAllGenes = clusterProfiler::read.gmt("../../Jansen_AllGenes.gmt")
AmitAllGenes = clusterProfiler::read.gmt("../../Amit_AllGenes.gmt")
ZhangAllGenes = clusterProfiler::read.gmt("../../Zhang_AllGenes.gmt")  

HumanTILs_ForScoring = c(Arrange_ForScoring(Human_TILs_Sigs[c(1,2)],JansenAllGenes),
                         Arrange_ForScoring(Human_TILs_Sigs[c(3,4)],AmitAllGenes),
                         Arrange_ForScoring(Human_TILs_Sigs[c(5)],ZhangAllGenes))
HumanTILs_ForScoring = HumanTILs_ForScoring[c(2,5,3,4,1)]

# max(-log10(OverRepresent_PVal(HumanTILs_ForScoring,TCellStateSigs)))

Hacohen_DEGs = msigdb::read.gmt("../../Hacohen_StateDEGs_Human.gmt")$genesets


TCellStateSigs = c(msigdb::read.gmt("../../MurineSigs.gmt")$genesets[1:4],
                   msigdb::read.gmt("../../YellowFever_EffectorMemory_Exc.gmt")$genesets[c(1,3)],
                   msigdb::read.gmt("../../YellowFever_EffectorMemory_RNASeq_v4Exc.gmt")$genesets[c(1,3)])

Jansen_Hacohen = PicnicPlot(HumanTILs_ForScoring,Hacohen_DEGs,FlipVertical = FALSE,UpperPVal = 61,UpperOddsRatio = 8.54)
Jansen_Hacohen$PicnicPlot


Jansen_TCellStateSigs = PicnicPlot(HumanTILs_ForScoring,TCellStateSigs,FlipVertical = FALSE,UpperPVal = 61,UpperOddsRatio = 8.54)
Jansen_TCellStateSigs$PicnicPlot

cowplot::plot_grid(Jansen_Hacohen$PicnicPlot,Jansen_TCellStateSigs$PicnicPlot,
...
pdf("5A_Overrepresentation_HumanTILs_v4.pdf",width = 15*Scale,height = 4.3*Scale)
cowplot::plot_grid(Jansen_Hacohen$PicnicPlot,Jansen_TCellStateSigs$PicnicPlot,
                   ncol = 2,align = "hv",rel_widths = c(1,1))
dev.off()

pdf("5A_Overrepresentation_HumanTILs_v4Legend.pdf",width = 15*Scale,height = 8*Scale)
cowplot::plot_grid(Jansen_Hacohen$LegendPlot,Jansen_TCellStateSigs$LegendPlot,
                   ncol = 2,align = "hv",rel_widths = c(1,1))
dev.off()