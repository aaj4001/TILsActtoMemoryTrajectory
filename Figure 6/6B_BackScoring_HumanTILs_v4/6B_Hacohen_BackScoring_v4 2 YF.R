...
DEGList = msigdb::read.gmt("../../HumanTILsSigs.gmt")$genesets

DEGList_UpDown = lapply(DEGList,function(x) list(Up = x,Down = NULL))
DEGList_UpDown = DEGList_UpDown[c(2,5,3,4,1)]

palette = c(brewer.pal(n = 8, name = 'Dark2')[1],"red","black","black",brewer.pal(n = 8, name = 'Dark2')[1])
...
for(i in 1:(7*length(DEGList_UpDown))) CombinedPlot[[i]] = CombinedPlot[[i]] + ylim(c(-1.2,1.5)) + theme(axis.text = element_text(size = 6))
...
Scale = 1.9
pdf("4B_Hacohen_BackScoring_v4 2 YF.pdf",width = 6.55*Scale,height = 4*Scale)
