...
DEGList = msigdb::read.gmt("../../Hacohen_StateDEGs_Human.gmt")$genesets
...
load("../../KineticSets_v3_2YF.rda")

# KineticLists$YF_Effector$PlotType="Replicate_Line"
...
# 
# for(i in 1:6) CombinedPlot[[i]] = CombinedPlot[[i]] + ylim(c(-1.5,1.5)) + theme(axis.text = element_text(size = 6))
# for(i in 7:12) CombinedPlot[[i]] = CombinedPlot[[i]] + ylim(c(-2.5,1.5)) + theme(axis.text = element_text(size = 6))
# for(i in 13:18) CombinedPlot[[i]] = CombinedPlot[[i]] + ylim(c(-3.5,1.5)) + theme(axis.text = element_text(size = 6))
# for(i in 19:24) CombinedPlot[[i]] = CombinedPlot[[i]] + ylim(c(-3,3.2)) + theme(axis.text = element_text(size = 6))
# 
# 
# for(i in 1:(6*(length(DEGList_UpDown)-1))) CombinedPlot[[i]] = CombinedPlot[[i]] + theme(axis.line.x = element_blank(),axis.ticks.x = element_blank(),axis.text.x = element_blank())
# for(i in seq(1,(6*length(DEGList_UpDown)))[-seq(1,(6*length(DEGList_UpDown)),6)]) CombinedPlot[[i]] = CombinedPlot[[i]] + theme(axis.text.y = element_blank())
# 
# Scale = 1.9
# pdf("Validation_MouseStates_BackScoring.pdf",width = 6.55*Scale,height = 3.5*Scale)
# # plot_grid(plotlist = CombinedPlot,align = "hv",ncol = 5,rel_widths = c(0.8,1,1,1,1.9))
# plot_grid(plotlist = CombinedPlot,align = "hv",ncol = 6,rel_widths = c(0.7,1,1,1,1,0.5))
# dev.off()
